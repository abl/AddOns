...
function TotemTimers.SetNumWeaponTimers()
    ...
    Timers[8].nrOfTimers = (role == 2) and 2 or 1
...
function TotemTimers.WeaponEvent(self, event, ...)
	...
	elseif event == "UNIT_INVENTORY_CHANGED" then
		...
		if offHandWeapon ~= lastOhWeapon and offHandWeapon ~= 0 then
			local enchant, texture 
			if not TotemTimers.ActiveProfile.LastOffEnchants[offHandWeapon] then
				enchant = ""
                texture = SpellTextures[SpellIDs.FlametongueWeapon]
			else
                enchant = TotemTimers.ActiveProfile.LastOffEnchants[offHandWeapon][1]
                texture = TotemTimers.ActiveProfile.LastOffEnchants[offHandWeapon][2]
            end
